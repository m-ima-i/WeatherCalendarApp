...
fun AddEventDialog(
    ...
    onDismiss: () -> Unit,
    onConfirm: (CreateEventReq) -> Unit,
    initialStartTime: LocalTime? = null,
    initialEndTime: LocalTime? = null
) {
    var title by remember { mutableStateOf("") }
    // 初期時刻が指定されたらタイムライン経由とみなし、終日OFFで起動する
    var isAllDay by remember { mutableStateOf(initialStartTime == null) }
                    ...
                    if (eventColors.isNotEmpty()) {
                        Column(
                            ...
                            Row(
                                ...
                                // Google Calendar Colors APIのパレット
                                eventColors.entries
